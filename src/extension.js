...
function editorFindNearestBookmark(documentUri, treeDataProvider, anchor, overrideStrategy) {
    ...
    function strategyNearestBookmark(previous, current) {
        ...
        return Math.abs(focusLine - current.location.range.start.line) <= Math.abs(focusLine - previous.location.range.start.line)
            ? current
            : previous;
    }
    ...
    function strategyLastKnownBookmark(previous, current) {
        ...
        return focusLine >= current.location.range.start.line &&
            focusLine - current.location.range.start.line <= focusLine - previous.location.range.start.line
            ? current
            : previous;
    }
    ...
    let focusBookmark = treeDataProvider.getChildren(root).reduce((prevs, current) => followMode(prevs, current), null);
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand("inlineBookmarks.jumpToNext", () => {
            ...
            if (treeView.visible && treeView.selection.length && lineMode === "selected-bookmark") {
                //treeview is visible and item selected.  If lineMode, ignore the current selected bookmark and rely on "chapter" below.
